...
OUTPUT_JSON_PATH = './preds/pt_preds.json'
...
def create_prompt(raw_text):
    return f"""Abaixo, você encontrará instruções descrevendo uma tarefa, juntamente com algumas informações básicas.
    Escreva uma resposta que responda corretamente à pergunta. Antes de responder, pense cuidadosamente sobre a pergunta para garantir a precisão da sua resposta.
    ### Instruções:
    Você é falante nativo de português. A seguinte frase contém uma expressão idiomática.
    Forneça uma paráfrase da frase inteira, preservando o significado original, substituindo a expressão idiomática por uma reformulação, usando sinônimos e modificando sua estrutura. Seja criativo.
    Apresente sua paráfrase da seguinte forma, sem adicionar comentários:
    A paráfrase é:
    1. <paráfrase>.
