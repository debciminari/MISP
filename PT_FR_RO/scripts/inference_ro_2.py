...
INPUT_JSON_PATH = './test_sets/ro_test.blind.json'
OUTPUT_JSON_PATH = './preds/ro_preds_2-task.json'
...
def create_prompt(raw_text):
    return f"""
### Instrucțiuni:
Ești vorbitor nativ de limba română. Reformulează propoziția urmând pașii de mai jos:
- Citește cu atenție propoziția și asigură-te că îi înțelegi pe deplin sensul.
- Exprimă ideea principală a propoziției cu propriile cuvinte: folosește sinonime, modifică structura propoziției și gramatica.
- Compară rescrierea ta cu propoziția originală pentru a verifica dacă sensul este păstrat.
Formatează-ți răspunsul după cum urmează:
Rescrierea este:
1)
